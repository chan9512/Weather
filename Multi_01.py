from urllib.request import urlopen
from urllib.parse import urlencode, unquote, quote_plus
import urllib
import requests
import json
import pandas as pd
from bs4 import BeautifulSoup
from datetime import date, timedelta, datetime
import time


url = 'http://apis.data.go.kr/1360000/TyphoonInfoService/getTyphoonInfo'




Queryparamss = '?' + 'serviceKey=' + '%2B6LwXwdSiXfBWg2A2q8IXzUCjGP13kzdct07M%2Bu6z9a%2BtwEhnndllUmg%2B9dzpW9ggINqxPfYck050bxzhUAPjw%3D%3D' \
                + '&pageNo=' + '1' \
                + '&numOfRows=' + '10'\
                + '&dataType=' + 'XML' \
                + '&fromTmFc=' + '20200420' \
                + '&toTmFc=' + '20200420'




url = url + Queryparamss
results = requests.get(url)
bs_obj = BeautifulSoup(results.text, "html.parser")

rstlist = []
resultmsg = bs_obj.find_all('resultmsg')
for code in resultmsg:
    rstlist.append(code.text)

print(bs_obj)

if rstlist[0]=='NO_DATA':
    print("현재 태풍 X ")
else:
    print(bs_obj)



today = date.today()
now = datetime.now()

url2 = 'http://apis.data.go.kr/1360000/MidFcstInfoService/getMidTa'

x_now=int(str(now.hour)+str(now.minute)) #현재 시간+분/ 인트형

if x_now>=600 and x_now<1800:
    today_string = today.strftime('%Y%m%d') + '0600'
    Queryparams = '?' + 'serviceKey=' + '%2B6LwXwdSiXfBWg2A2q8IXzUCjGP13kzdct07M%2Bu6z9a%2BtwEhnndllUmg%2B9dzpW9ggINqxPfYck050bxzhUAPjw%3D%3D' \
                    + '&pageNo=' + '1' \
                    + '&numOfRows=' + '10'\
                    + '&dataType=' + 'XML' \
                    + '&regId=' + '11B20201' \
                    + '&tmFc=' + today_string
    print('0600~1800')
else:
    today_string = today.strftime('%Y%m%d') + '1800'
    Queryparams = '?' + 'serviceKey=' + '%2B6LwXwdSiXfBWg2A2q8IXzUCjGP13kzdct07M%2Bu6z9a%2BtwEhnndllUmg%2B9dzpW9ggINqxPfYck050bxzhUAPjw%3D%3D' \
                  + '&pageNo=' + '1' \
                  + '&numOfRows=' + '10' \
                  + '&dataType=' + 'XML' \
                  + '&regId=' + '11B20201' \
                  + '&tmFc=' + today_string
    print('1800~0600')


url2 = url2 + Queryparams
result = requests.get(url2)

bs_objs = BeautifulSoup(result.text, "html.parser")

CommerceInfor = {}
Max_3list = []
Min_3list = []

Max3 = bs_objs.find_all('tamax3')
Min3 = bs_objs.find_all('tamin3')

for code in Max3:
    Max_3list.append(code.text)
for code in Min3:
    Min_3list.append(code.text)

CommerceInfor['3일뒤 최고기온'] = Max_3list
CommerceInfor['3일뒤 최저기온'] = Min_3list


df = pd.DataFrame(CommerceInfor)
df = df[['3일뒤 최고기온','3일뒤 최저기온']]

#print(df)
#print(bs_obj)


TM3 = int(Max_3list[0])
Tm3 = int(Min_3list[0])

print('<인천지역>')
if 33<= TM3 <35:
    print('3일뒤 폭염주의보, 3일뒤 최고기온은: '+ str(TM3) + '도')
elif 35<= TM3:
    print('3일뒤 폭염경보, 3일뒤 최고기온은: ' + str(TM3) + '도')
elif -15< Tm3 <= -12:
    print('3일뒤 한파주의보, 3일뒤 최저기온은: ' + str(Tm3) + '도')
elif Tm3 <= -15:
    print('3일뒤 한파경보, 3일뒤 최저기온은: ' + str(Tm3) + '도')
else:
    print('3일뒤 폭염특보X, 3일뒤 최고기온은: ' + str(TM3) + '도')
    print('3일뒤 한파특보x, 3일뒤 최저기온은: ' + str(Tm3) + '도')